"""Frozen independent-agent instrument-use qualification.
...
REPORT_SCHEMA_VERSION = "chemworld-agent-instrument-use-report-0.2"
QUALIFICATION_ID = "agent-instrument-use-v1"
METHOD_ID = "agent_instrument_use_interactive_codex_sol_medium_v1"

PROTOCOL = Path("protocols/agent-instrument-use.md")
CONTROLLED_FORK_EVIDENCE = Path("evidence/reports/controlled-world-forks.json.gz")
COMPOSITION_EVIDENCE = Path("evidence/reports/composition-qualification.json.gz")
EVALUATOR_SOURCE = Path("src/chemworld/eval/agent_instrument_use.py")
...
def validate_launch_preconditions(
    ...
    *,
    ...
    protocol_path = _resolve_repository_path(root, PROTOCOL)
    if not protocol_path.is_file():
        errors.append("frozen public protocol is missing")
    else:
        protocol = protocol_path.read_text(encoding="utf-8")
        required_protocol_fragments = (
            "frozen before the passing execution",
            "one complete agent lifecycle",
            "Between 1 and 16 typed actions",
            "full submitted-action trace replays exactly",
            "Raw provider responses, credentials and private reasoning are excluded",
        )
        missing = [item for item in required_protocol_fragments if item not in protocol]
        if missing:
            errors.append("public protocol drifted: " + ", ".join(missing))
    ...
    return {
        ...
        "protocol": _source_binding(root, PROTOCOL),
...
def _public_evidence_binding(root: Path, relative_path: Path) -> dict[str, Any]:
    path = _resolve_repository_path(root, relative_path)
    if not path.is_file():
        raise CompleteAgentQualificationError(f"public evidence is missing: {relative_path}")
    return {
...
def resolve_existing_evidence(repository_root: str | Path) -> dict[str, Any]:
    """Resolve prerequisite results through the stable public evidence files."""

    root = Path(repository_root).resolve()
    u04_path = _resolve_repository_path(root, CONTROLLED_FORK_EVIDENCE)
    u05_path = _resolve_repository_path(root, COMPOSITION_EVIDENCE)
    u04_report = _read_json_object(u04_path)
    u05_report = _read_json_object(u05_path)
    u04_binding = _public_evidence_binding(root, CONTROLLED_FORK_EVIDENCE)
    u05_binding = _public_evidence_binding(root, COMPOSITION_EVIDENCE)
